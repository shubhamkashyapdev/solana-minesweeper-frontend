...
export function RotateCard(slot: CardSlot, texture: PIXI.Texture): void {
  ...
  ticker.start()
  ticker.add(() => {
    slot.sprite.setTransform(
      slot.sprite.position.x,
      slot.sprite.position.y,
      slot.sprite.scale.x + rot
    )
...
export function ShowMultiplierPopup(slot: CardSlot) {
  ...
  var text = new PIXI.Text(
    "+" + Round(slot.GetCard().worth * (board.mode as number), 1) + "X",
    style
  )
  app.stage.addChild(text)
  text.x =
    slot.sprite.x + slot.sprite.width - text.width / 2 - RandomFromTo(0, 100)
  text.y =
    slot.sprite.y + slot.sprite.height - text.height / 2 - RandomFromTo(0, 100)
  text.alpha = 0
